    ...
    fn tick(
        ...
        if self.entries.is_empty() || (self.now - self.last_store) >= 1.0 / DELAY_STORE_FPS {
            let tex = self
                .pool
                .pop()
                .unwrap_or_else(|| create_rgba8(gl, dims.0, dims.1));
...
/// An RGBA8 texture of the given size for a delay-ring slot.
fn create_rgba8(gl: &Gl, w: i32, h: i32) -> WebGlTexture {
    let tex = gl.create_texture().expect("failed to create ring texture");
    gl.bind_texture(Gl::TEXTURE_2D, Some(&tex));
    set_texture_params(gl);
